...
InputHandle = namedtuple("InputHandle", ["real_data", "embedding_ids", "no_target_data",
                                         "no_target_ids", "cns_code", "seq_len"])
...
class UNet(object):
    def __init__(self, experiment_dir=None, experiment_id=0, batch_size=16, input_width=256, output_width=256,
                 ...
                 Lcategory_penalty=1.0, embedding_num=7, embedding_dim=128, input_filters=1, output_filters=1,
                 cns_encoder_dir="", cns_embedding_size=128, lstm_num_units=128, z_dim=32):
    ...
    def cns_encoder(self, cns_code, seq_len, reuse=False):
        with tf.variable_scope("cns_encoder"):
            if reuse:
                tf.get_variable_scope().reuse_variables()
            # src_masks
            src_masks = tf.equal(cns_code, 0)  # (N, T1)
    ...
    def generator(self, images, embedding_ids, cns_code, seq_len, inst_norm, is_training, reuse=False):
        ...
        one_hot = tf.reshape(tf.one_hot(indices=embedding_ids, depth=self.embedding_num),
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        fake_B, encoded_real_A = self.generator(real_A, embedding_ids, cns_code, seq_len,
        ...
        if no_target_source:
            ...
            no_target_B, encoded_no_target_A = self.generator(no_target_A, no_target_ids, cns_code=cns_code, seq_len=seq_len,
            ...
            no_target_D, no_target_D_logits, no_target_category_logits = self.discriminator(no_target_AB, is_training=is_training, reuse=True)
    ...
    def validate_model(self, val_iter, epoch, step):
        ...
        # cns_code, seq_len, labels, images = val_iter.items()[0]
            fake_imgs, real_imgs, d_loss, g_loss, l1_loss = self.generate_fake_samples(images, labels, cns_code, seq_len)
    ...
    def infer(self, source_obj, embedding_ids, model_dir, save_dir):
        ...
        for cns_code, seq_len, labels, source_imgs in source_iter:
            re = self.generate_fake_samples(source_imgs, labels, cns_code, seq_len)
            fake_imgs = re[0]
            real_imgs = re[1]
            img_path = os.path.join(save_dir, "inferred_%04d.jpg" % count)
            save_imgs([fake_imgs,real_imgs],count)
            # imageio.imwrite(img_path, fake_imgs.squeeze())
            count += 1
        '''
        ...
        for step_idx in range(len(alphas)):
            ...
            if len(batch_buffer):
                save_concat_images(batch_buffer,
                                   os.path.join(save_dir, "frame_%02d_%02d_step_%02d.jpg" % (
    ...
    def train(self, lr=0.0002, epoch=100, schedule=10, resume=True, flip_labels=False,
        ...
        # cns_optimizer = tf.train.AdamOptimizer(0.0002, beta1=0.5).minimize(loss_handle.g_loss, var_list=cns_vars)
